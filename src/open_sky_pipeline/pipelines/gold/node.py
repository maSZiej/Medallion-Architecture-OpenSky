...
def _write_to_gold(gold_df: DataFrame, gold_path: str,spark:SparkSession):
...
def _write_to_db(df: DataFrame, target_table: str):
    ...
    jdbc_url = "jdbc:postgresql://postgres:5432/OpenSky_docker"
# 
    df.write.mode("overwrite").jdbc(
        url=jdbc_url, table=target_table, properties=credentials
    )

def aggregate_data(df: DataFrame,df_hist: DataFrame) -> tuple[DataFrame,DataFrame,DataFrame,DataFrame]:
    df_fact = df.select(
                "callsign",
                "last_contact_h",
                "time_position_h",
    ...
    df_aircraft_dim = df.select("icao24", "origin_country", "aircraft_category")
            ...
            # historical data
    df_all = (
                df_hist.groupBy(col("ingestion_timestamp"))
                .count()
    ...
    df_velocity = (
                ...
                .orderBy(col("ingestion_timestamp"), ascending=False)
                .select(
                    "ingestion_timestamp", "avg_velocity", "avg_baro_alt", "count_flying"
                )
            )
            ##################
    df_all = (
    ...
    df_cat = (
                ...
                .orderBy(
                    [col("ingestion_timestamp"), col("vertical_category")], ascending=False
                )
            )
    return df_fact, df_aircraft_dim, df_all,  df_cat

def gold_layer(*args, **kwargs):
    spark = SparkSession.builder.getOrCreate()
    # guard
    if not DeltaTable.isDeltaTable(spark, "s3a://meddalion/silver/aircraft_hist"):
        return True
    else:
        df = (
        ...
        df_fact, df_aircraft_dim, df_all,  df_cat = aggregate_data(df,df_hist)
        ...
        # savings
        #################
        df_fact.write.mode("overwrite").format("delta").save(
            "s3a://meddalion/gold/fact_table"
        )
        if not DeltaTable.isDeltaTable(spark, "s3a://meddalion/gold/dim_table"):
            _write_to_gold(df_aircraft_dim, "s3a://meddalion/gold/dim_table",spark)
        else:
        ...
        _write_to_gold(df_all, "s3a://meddalion/gold/KPI_for_timestamps",spark)
        _write_to_gold(df_cat, "s3a://meddalion/gold/KPI_for_cat&ts",spark)
