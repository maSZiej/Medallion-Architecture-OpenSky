...
def test_aggregate_data(sample_df, sample_df_hist):
    ...
    assert all([df_fact.count(), df_aircraft_dim.count(), df_all.count(), df_cat.count()]) >0
    ...
    expected_types=[expected_fact_types, expected_dim_types, expected_kpi_types, expected_cat_types]
